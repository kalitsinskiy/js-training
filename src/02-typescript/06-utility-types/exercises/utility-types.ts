...
// Run this file with: npx ts-node --project config/tsconfig.json src/02-typescript/06-utility-types/exercises/utility-types.ts
...
function hasPermission(role: keyof RoleToPermissions, action: string): boolean {
  return roleToPermissions[role].includes(action);
}
...
const focusEvent: FocusEvents = 'blur';
...
const a: A = { title: 'Partial' };
const b: B = { id: 'u1', name: 'Alice', email: 'alice@example.com' };
const c: C = { id: 'a1', title: 'Pick' };
const d: D = {
  id: 'a1',
  title: 'Omit',
  tags: [],
