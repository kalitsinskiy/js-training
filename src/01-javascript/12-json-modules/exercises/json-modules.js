...
const filteredJson = JSON.stringify(sensitiveUser, ['id', 'username', 'email', 'role']);
...
console.log('createdAt is Date:', parsedEvent.createdAt instanceof Date);
console.log('updatedAt is Date:', parsedEvent.updatedAt instanceof Date);
...
const json1 = JSON.stringify(resource);
console.log(json1); // Should be: {"id":1,"name":"Users"}
...
function parseConfig(jsonString) {
  ...
  if (typeof config.host !== 'string') {
    throw new Error('Invalid type: host must be string');
  }
  if (typeof config.port !== 'number') {
    throw new Error('Invalid type: port must be number');
  }
  if (typeof config.debug !== 'boolean') {
    throw new Error('Invalid type: debug must be boolean');
  }
  return config;
}
