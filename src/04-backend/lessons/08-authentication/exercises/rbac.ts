...
const rolePermissions: Record<string, Permission[]> = {
  // Keep the permission map explicit so each role's allowed actions stay easy to audit.
  admin: [
    'rooms:create',
    'rooms:read',
    'rooms:update',
...
function requirePermissions(...permissions: Permission[]): Middleware {
  return (req: Request, next: NextFunction): Response => {
    ...
    const grantedPermissions = rolePermissions[req.user.role] ?? [];
...
function isOwnerOrAdmin(getOwnerId: (req: Request) => string): Middleware {
  return (req: Request, next: NextFunction): Response => {
    ...
    // Ownership checks are a common fallback when role checks alone are too coarse.
...
function check(label: string, expectedStatus: number, fn: () => Response): void {
  try {
    ...
    if (err instanceof Error && err.message === 'Not implemented') {
      ...
      console.log(`${label}: FAIL (threw: ${getErrorMessage(err)})`);
    }
  }
}
