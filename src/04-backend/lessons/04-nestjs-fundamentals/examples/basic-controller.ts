...
// ============================================
// Run: npx ts-node src/04-backend/lessons/04-nestjs-fundamentals/examples/basic-controller.ts
//
...
import { Controller, Get, Post, Param, Body, Module, HttpCode } from '@nestjs/common';
...
class BooksController {
  ...
  @Get(':id')
  findOne(@Param('id') id: string): Book | { message: string } {
    console.log(`GET /books/${id}`);
    const book = this.books.find(b => b.id === id);
    if (!book) {
      return { message: `Book with id "${id}" not found` };
    }
    return book;
  }
